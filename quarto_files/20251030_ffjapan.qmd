---
title: "20251030_FamaFrenchFactors_in-Japan"
format: gfm
execute:
    warning: false
    message: false
...
これらの記事を参考に、RでJ-Quantからデータを取得する関数を書いていきます。事前にJ-Quantにユーザー登録して、Refresh tokenを発行するようにしてください。僕は、`authinfo.r`というファイルに認証情報を以下の形で入れてあります。
```{r}
jquant <- list(
